---
name: taskflow
description: "Task Flow backlog management via `task.*` tools. Use when the user explicitly wants durable tasks, TODO/backlog management, kanban-like flows, decomposition into multiple tasks, dependencies, ownership changes, or employee handoff."
...
- maintain Project Knowledge Spine flow docs and focused task handoff/review/evidence docs
...
- hand off work between employees with durable context
...
2. For multi-step work, create a flow first with `task.flow.create`.
   - Flow docs are created lazily/defaulted for `brief`, `plan`, `spec`, `decisions`, and `status`.
...
6. For reassignment or handoff, use `task.update`:
   ...
   - `status=review` when ready for employee review
   - `status=blocked` when waiting for operator input, approval, or external state
...
10. When the user wants review work for a specific employee/profile, inspect `task.review.list` instead of approximating it from owner filters.
...
12. When a brief/plan/spec/decision/status update should survive across agents, use `task.doc.put`; use `task.doc.confirm` when the current revision has been accepted.
...
17. When the user wants durable discussion or review notes on a task, use `task.comment.add` and `task.comment.list`.
18. When the user needs operator-style repair visibility for stuck background work, inspect `task.stale.list` first and only then use `task.stale.sweep`.
...
- Do not hand Task Flow work to a human owner. Keep work assigned to an employee and use comments/docs/blockers to capture operator questions or approvals.
...
- Before a background task ends in `review`, `blocked`, `completed`, or `failed`, add a durable `task.comment.add` note so the task keeps human-readable context beyond raw events and run logs.
...
- Employee handoff:
  call `task.update` on the current task with `owner_type=employee`, `owner_ref=<employee_id>`, and either `status=review`, `status=blocked`, or `status=todo` depending on why the handoff is happening.
...
- Update durable docs:
  call `task.doc.put` with `scope_type=flow|task`, `scope_id`, `document_key=brief|plan|spec|decisions|status|handoff|notes|review|evidence`, and the complete document body.
- Confirm docs:
...
- Inspect reviewer inbox:
  call `task.review.list` with `actor_type=employee` and `actor_ref=<employee_id>` for employee reviewers to see tasks currently waiting in review.
- Inspect all reviewer inboxes:
  call `task.review.list` with `all_reviewers=true` to see review tasks assigned to employees before deciding a flow is clear.
