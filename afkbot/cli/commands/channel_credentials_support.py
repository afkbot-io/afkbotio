    ...
    *,
    ...
    return cast(
        ...
        _select_labeled_option(
            title=msg(
                lang,
                en=f"{app_label_en}: Credentials",
                ru=f"{app_label_ru}: Учётные данные",
            ),
            text=msg(
                lang,
    ...
    *,
    ...
    typer.echo(
        msg(
            lang,
            en=(
                "You will need the PartyFlow bot bearer token from Integrations -> Bots. "
                "The outgoing webhook signing secret is optional: leave it blank to accept "
                "PartyFlow webhooks without signature validation."
            ),
            ru=(
                "Понадобится bearer token бота из Integrations -> Bots. "
                "Секрет подписи исходящего webhook необязателен: оставьте его пустым, "
                "чтобы принимать webhook PartyFlow без проверки подписи."
            ),
        )
    )
    token = resolve_channel_secret(
        ...
        prompt_ru="Токен бота PartyFlow",
        lang=lang,
        existing_configured=_PARTYFLOW_BOT_TOKEN in existing,
        required=True,
        detail_en="Paste the PartyFlow bot token in the form `fri_bot_...`.",
        detail_ru="Вставьте токен бота PartyFlow в формате `fri_bot_...`.",
    )
    signing_secret = resolve_channel_secret(
        value=None,
        interactive=interactive,
        prompt_en="PartyFlow webhook signing secret",
        prompt_ru="Секрет подписи webhook PartyFlow",
        lang=lang,
        existing_configured=_PARTYFLOW_WEBHOOK_SIGNING_SECRET in existing,
        required=False,
        detail_en=(
            "Optional security check. Paste the signing secret generated by PartyFlow to validate "
            "webhook signatures, or leave this blank to skip signature validation."
        ),
        detail_ru=(
            "Необязательная проверка безопасности. Вставьте секрет подписи, который PartyFlow "
            "сгенерировал для webhook, или оставьте поле пустым, чтобы не проверять подпись."
        ),
    )
    if token is not None:
        _upsert_app_secret(
