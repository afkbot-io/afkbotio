    ...
    return cast(
        ...
        _select_labeled_option(
            title=msg(
                ...
                ru=f"{app_label_ru}: Учетные данные",
            ),
            text=msg(
                lang,
                en="Credentials for this channel already exist. Keep them or update them now?",
                ru="Для этого канала учетные данные уже настроены. Оставить их или обновить сейчас?",
            ),
            options=(
                (
                    "keep",
                    msg(lang, en="Keep current credentials", ru="Оставить текущие учетные данные"),
                ),
                (
                    "update",
                    msg(lang, en="Update credentials now", ru="Обновить учетные данные сейчас"),
                ),
            ),
            default="keep",
            lang=lang,
    ...
    *,
    ...
    typer.echo(
        msg(
            ...
            ru=f"Для этого канала будет использован профиль учетных данных `{credential_profile_key}`.",
        )
    )
    typer.echo(
        msg(
    ...
    *,
    ...
    typer.echo(
        msg(
            ...
            ru=(
                "Понадобятся bearer token бота из Integrations -> Bots и секрет подписи исходящего webhook, "
    ...
    signing_secret = resolve_channel_secret(
        ...
        detail_ru="Вставьте секрет подписи, который PartyFlow сгенерировал для этой подписки на исходящий webhook.",
    )
    if token is not None:
        _upsert_app_secret(
            settings=settings,
