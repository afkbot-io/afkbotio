...
def _build_task_flow_prompt_overlay(
    *,
    ...
    lines = [
        ...
        "For broader or approval-sensitive work, persist the plan in a flow document with task.doc.put using document_key=plan, update document_key=status as the project state changes, and keep comments as progress notes rather than the only source of truth.",
        "Before the task ends in review, blocked, completed, failed, employee handoff, or operator-blocked state, add a durable task.comment.add note that summarizes outcome, remaining work, and any review context.",
        ...
        "When the current task needs help from another specialist, prefer task.delegate to create a self-contained task for another employee and leave a durable handoff trail.",
