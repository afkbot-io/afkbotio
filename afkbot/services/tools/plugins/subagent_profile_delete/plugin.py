"""Tool plugin for subagent.profile.delete."""
...
class SubagentProfileDeleteParams(RoutedToolParameters):
    """Parameters for subagent.profile.delete tool."""

    name: str = Field(min_length=1, max_length=128)


class SubagentProfileDeleteTool(ToolBase):
    ...
    async def execute(self, ctx: ToolContext, params: ToolParameters) -> ToolResult:
        payload = SubagentProfileDeleteParams.model_validate(params.model_dump())
...
def create_tool(settings: Settings) -> ToolBase:
    """Create subagent.profile.delete tool instance."""

    return SubagentProfileDeleteTool(settings=settings)
