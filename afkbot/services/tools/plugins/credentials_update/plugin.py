"""Tool plugin for credentials.update."""
...
class CredentialsUpdateParams(ToolParameters):
    """Parameters for credentials.update tool."""
...
class CredentialsUpdateTool(ToolBase):
    """Update encrypted credential binding for one app/profile/slug."""

    name = "credentials.update"
    description = "Update encrypted credential binding for trusted callers. Returns metadata only."
    parameters_model = CredentialsUpdateParams

    def __init__(self, settings: Settings) -> None:
        self._settings = settings

    async def execute(self, ctx: ToolContext, params: ToolParameters) -> ToolResult:
        payload = CredentialsUpdateParams.model_validate(params.model_dump())
...
def create_tool(settings: Settings) -> ToolBase:
    """Create credentials.update tool instance."""

    return CredentialsUpdateTool(settings=settings)
