...
class TaskCommentListTool(ToolBase):
    ...
    async def execute(self, ctx: ToolContext, params: ToolParameters) -> ToolResult:
        payload = (
            params
            if isinstance(params, TaskCommentListParams)
            else TaskCommentListParams.model_validate(params)
        )
        ...
        try:
            ...
            return ToolResult(
                ok=True, payload={"task_comments": [item.model_dump(mode="json") for item in items]}
            )
