...
class TaskEventListTool(ToolBase):
    ...
    async def execute(self, ctx: ToolContext, params: ToolParameters) -> ToolResult:
        payload = (
            params
            if isinstance(params, TaskEventListParams)
            else TaskEventListParams.model_validate(params)
        )
        ...
        try:
            ...
            return ToolResult(
                ok=True, payload={"task_events": [item.model_dump(mode="json") for item in items]}
            )
